...
<body>
	<nav class="breadcrumb">
		...
		用户申请列表 <a class="btn btn-success radius r" style="line-height: 1.6em; margin-top: 3px"
	...
	<div class="page-container">
		<table class="table table-border table-bordered table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="8">用户申请列表</th>
				</tr>
				<tr class="text-c">
					<th>申请人</th>
					<th>标题</th>
			...
			<c:forEach items="${applysList}" var="applys">
				<tr class="text-c">
					<td>${applys.realname}</td>
					<td>${applys.title}</td>
					<td>${applys.catename}</td>
					<td>${applys.money}</td>
					<td>${applys.contents}</td>
					<td>${applys.addtime}</td>
					<td>${applys.status}</td>
					<td>
					<c:if test="${applys.status eq '待审核'}">
					<a href="applys/confirm.action?id=${applys.applysid}">通过审核</a>&nbsp;&nbsp;
