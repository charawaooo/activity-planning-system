...
<body>
	<nav class="breadcrumb">
		<i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>用户申请管理 <span class="c-gray en">&gt;</span>
		用户申请查询 <a class="btn btn-success radius r" style="line-height: 1.6em; margin-top: 3px"
	...
	<div class="page-container">
		<table class="table table-border table-bordered table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="8">用户申请查询</th>
				</tr>
				<tr class="text-c">
			...
			<c:forEach items="${applysList}" var="applys">
				<tr class="text-c">
					<td class="center">${applys.realname}</td>
					<td class="center">${applys.title}</td>
					<td class="center">${applys.catename}</td>
					<td class="center">${applys.money}</td>
					<td class="center">${applys.contents}</td>
		...
		<div class="text-c">
			<form action="applys/queryApplysByCond.action" name="myform" method="post">
				查询条件： <span class="select-box" style="width: 200px;"><select name="cond" class="select">
						<option value="usersid">按申请人查询</option>
						<option value="title">按标题查询</option>
						<option value="cateid">按申请类型查询</option>
						<option value="money">按申请金额查询</option>
						<option value="contents">按申请内容查询</option>
